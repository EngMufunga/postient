...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-lg-7">
            <form action="{{route('user.subscription.payment', $plan->id)}}" method="post">
                @csrf
                <input type="hidden" name="method_code">
                <input type="hidden" name="currency">
                <div class="card custom--card">
                    <div class="card-header">
                        <h5 class="card-title">@lang('Subscription Payment for') <span class="text--base">{{ $plan->name }}</span></h5>
                    </div>
                    <div class="card-body">
                        <div class="form-group">
                            <label class="form-label">@lang('Select Gateway')</label>
                            <select class="form-control form--control" name="gateway" required>
                                <option value="wallet">{{ showAmount(auth()->user()->balance, 2) }} {{__($general->cur_text)}}</option>
                        ...
                        <div class="mt-3 preview-details d-none">
                            <ul class="list-group">
                                <li class="list-group-item d-flex justify-content-between">
                                    ...
                                    <span><span class="min fw--500">0</span> {{__($general->cur_text)}} - <span
                                            class="max fw--500">0</span> {{__($general->cur_text)}}</span>
                                </li>
                                <li class="list-group-item d-flex justify-content-between">
                                    <span>@lang('Charge')</span>
                                    <span><span class="charge fw--500">0</span> {{__($general->cur_text)}}</span>
                                </li>
                                <li class="list-group-item d-flex justify-content-between">
                                    <span>@lang('Payable')</span> <span><span class="payable fw--500"> 0</span>
                                ...
                                <li class="list-group-item justify-content-between d-none in-site-cur">
                                    ...
                                    <span class="final_amo fw--500">0</span>
                                </li>
                        ...
                        @if(!$planCheck)
                        <p class="text--danger mt-3">@lang('Your current plan will be cancelled and overriden with this subscription package')</p>
                        @endif
                    </div>
                </div>
            </form>
    ...
    (function ($) {
        ...
        $('select[name=gateway]').on('change', function () {
            ...
            if (resource.currency != '{{ $general->cur_text }}') {
                var rateElement = `<span class="fw--500">@lang('Conversion Rate')</span> <span><span  class="fw--500">1 {{__($general->cur_text)}} = <span class="rate">${rate}</span>  <span class="base-currency">${resource.currency}</span></span></span>`;
